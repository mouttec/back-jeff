...
$address->address = $decodedData->billingAddress;
$billingAddress = $address->searchAddressByAddress($address);
if (count($billingAddress) != 0) {
	extract($billingAddress);
...
if (isset($decodedData->idCustomer)) {
    ...
    $address->idCustomer = $result['idCustomer'];
    $idBillingAddress = $address->createAddress($address);
    if ($idBillingAddress) {
    	$customer->idBillingAddress = $idBillingAddress;
